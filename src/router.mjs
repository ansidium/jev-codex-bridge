 ...
 */
export async function askJev({ prompt, current, contextTokens, models, previousPrompt, reasoningLevels = [] }) {
  ...
  const request = {
    state: {
      ...
      ...(previousPrompt ? { previous_request: previousPrompt.slice(-2000) } : {}),
    },
    questions: { ...QUESTIONS, model: questionForModels(models),
      ...(reasoningLevels.length ? { reasoning_effort: questionForEfforts(reasoningLevels) } : {}) },
  };
  try {
    ...
    return {
      ...
      request,
      response: result,
      reasoning: result.answers.reasoning_effort ?? null,
      metrics: {
