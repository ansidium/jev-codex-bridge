 ...
 */
export async function askJev({ prompt, current, contextTokens, contextEstimate, profiles, previousPrompt, conversation, continuation = false }) {
  ...
  try {
    ...
    const original = {
      state: {
        request: prompt,
        session: { approximate_context_tokens: contextTokens, ...(contextEstimate ? { context_estimate: contextEstimate } : {}), continuation },
