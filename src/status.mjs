...
/** Match Desktop request thread ids to the id available in its tool environment. */
export function codexStatusId(threadId) {
  return typeof threadId === "string" && /^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i.test(threadId)
    ? `codex-thread-${threadId.toLowerCase()}`
    : null;
}
