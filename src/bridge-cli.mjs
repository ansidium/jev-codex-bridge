...
const help = `Jev Codex Bridge
  install [--key-file PATH] [--port 18767] [--codex-home PATH]
          [--no-config] [--no-service] [--task-name NAME]
...
function service(home, action) {
  ...
  return run("powershell.exe", ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File",
    join(SOURCE, "scripts", "windows-service.ps1"), "-Action", action, "-BridgeHome", home, "-Node", process.execPath], SOURCE, 30_000);
}
...
export async function main(args) {
  ...
  if (command === "service") {
    if (!["start", "stop", "restart", "status", "remove"].includes(option)) throw new Error("Choose service start, stop, restart, status, or remove.");
  ...
  await withLock(home, async () => {
    if (command === "install") {
      ...
      saveJson(join(home, "settings.json"), { port, keyFile, taskName });
