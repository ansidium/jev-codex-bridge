...
const decision = (reason = "") => {
  ...
  if (reason.includes("cache-rebuild")) return "large-context guard";
...
export function formatExplanation(status) {
  ...
  return [
    ...
    row(`Current ${answers?.model ? "model" : "tier"}: ${(request?.session?.current_model ?? "unknown").toUpperCase()}`),
    ...
    row(),
    ...wrapped(`Recommended ${answers?.model ? "model" : "tier"}: `, recommendation.toUpperCase()),
