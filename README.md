...
Jev chooses a model from your account's catalog when you send a message. It also
chooses the reasoning effort, up to the level you selected in Codex. The rest of
the turn keeps that choice, including tool calls.
...
- Reads model IDs, descriptions and reasoning levels from the account catalog.
- Includes the previous user request so a short “go ahead” retains its context.
- Uses the frontier tier for difficult or ambiguous reasoning, even on short tasks.
...
- Avoids downgrading an established large conversation to reduce cache rebuilding.
  A fresh task can still start on a cheaper model.

Thresholds and capability guidance live in [src/config.mjs](src/config.mjs).
...
TypeSafe receives the current user text, up to 2,000 characters of the previous
