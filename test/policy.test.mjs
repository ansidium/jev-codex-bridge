...
import { QUESTIONS, questionForEfforts, shouldUseExactModel } from "../src/config.mjs";
...
test("reasoning question uses only levels supplied by the live model catalog", () => {
  const question = questionForEfforts([{ effort: "shallow", description: "Quick work" }, { effort: "deep", description: "Complex work" }]);
  assert.deepEqual(Object.keys(question.criteria), ["shallow", "deep"]);
});

test("initial instructions do not imply a previous model cache", () => {
  const initial = { ...base, current: "opus", contextTokens: 80000, hasPriorModel: false };
  assert.equal(decide({ ...initial, jev: sure("haiku") }).tier, "haiku");
  assert.equal(decide({ ...initial, jev: unsure("haiku") }).tier, "opus");
  assert.equal(decide({ ...initial, jev: sure("fable") }).tier, "fable");
  assert.equal(decide({ ...initial, hasPriorModel: true, jev: sure("haiku") }).tier, "opus");
});
